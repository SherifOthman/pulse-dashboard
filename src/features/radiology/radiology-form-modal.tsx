...
type Props = {
  isOpen: boolean
  onClose: () => void
  onSubmit: (dto: CreateRadiologyDto) => void
  isLoading?: boolean
  initial?: RadiologyDto | null
}

export function RadiologyFormModal({ isOpen, onClose, onSubmit, isLoading, initial }: Props) {
    ...
    <Modal.Backdrop isOpen={isOpen} onOpenChange={onClose}>
      <ModalContainer size="md">
        <ModalDialog>
          <ModalCloseTrigger />
          <ModalHeader>
            <ModalHeading>{initial ? 'تعديل مركز الأشعة' : 'إضافة مركز أشعة جديد'}</ModalHeading>
          </ModalHeader>
          <ModalBody className="flex flex-col gap-4">
            <div>
              <label className="text-sm font-medium text-foreground mb-1.5 block">الاسم *</label>
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="اسم مركز الأشعة" className="w-full" />
            </div>
            ...
            <div>
              ...
              <Select placeholder="اختر المحافظة" selectedKey={governorateId || null} onSelectionChange={(key) => { setGovernorateId(key as string); setCityId('') }}>
                <SelectTrigger>
                  <SelectValue />
                  <SelectIndicator />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select placeholder="اختر المدينة" selectedKey={cityId || null} onSelectionChange={(key) => setCityId(key as string)} isDisabled={!governorateId}>
                <SelectTrigger>
                  <SelectValue />
                  <SelectIndicator />
                </SelectTrigger>
            ...
            <div>
              ...
              <Input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="عنوان المركز" className="w-full" />
            </div>
            <div>
              <label className="text-sm font-medium text-foreground mb-1.5 block">الوصف</label>
              <Input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="وصف المركز" className="w-full" />
            </div>
          </ModalBody>
          <ModalFooter>
