  ...
  return (
    <div dir="rtl">
      <PageHeader
        ...
        subtitle={
          data
            ? `${data.totalCount} صيدلية مسجلة`
            : isLoading
              ? 'جاري التحميل...'
              : undefined
        }
        action={
          <Button variant="primary" onPress={() => navigate('/pharmacies/new')}>
      ...
      <Table variant="primary">
        <Table.ScrollContainer>
          <Table.Content
            aria-label="قائمة الصيدليات"
            onRowAction={(key) => navigate(`/pharmacies/${key}`)}
          >
            <Table.Header>
              <Table.Column isRowHeader className="text-right">الصيدلية</Table.Column>
                ...
                isError ? (
                  ...
                  <div className="flex flex-col items-center gap-3 py-14 text-muted">
                    ...
                    <span className="text-sm">
                      {hasActiveFilters ? 'لا توجد نتائج للفلاتر المحددة' : 'لا يوجد صيدليات مسجلة'}
                    </span>
                  </div>
                )
              }
