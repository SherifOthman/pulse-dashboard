...
export function LabsPage() {
  ...
  const { data, isLoading, isError } = labHooks.useList({
    page,
    pageSize: PAGE_SIZE,
  ...
  const deleteMutation = labHooks.useDelete()
  ...
  return (
    <div dir="rtl">
      <PageHeader
        ...
        subtitle={
          data
            ? `${data.totalCount} مختبر مسجل`
            : isLoading
              ? 'جاري التحميل...'
              : undefined
        }
        action={
          <Button variant="primary" onPress={() => navigate('/labs/new')}>
      ...
      <Table variant="primary">
        <Table.ScrollContainer>
          <Table.Content
            aria-label="قائمة المختبرات"
            onRowAction={(key) => navigate(`/labs/${key}`)}
          >
            <Table.Header>
              <Table.Column isRowHeader className="text-right">المختبر</Table.Column>
                ...
                isError ? (
                  ...
                  <div className="flex flex-col items-center gap-3 py-14 text-muted">
                    ...
                    <span className="text-sm">
                      {hasActiveFilters ? 'لا توجد نتائج للفلاتر المحددة' : 'لا يوجد مختبرات مسجلة'}
                    </span>
                  </div>
                )
              }
